...
bool msgs__msg__detection_with_gps__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("msgs.msg._detection_with_gps.DetectionWithGPS", full_classname_dest, 45) == 0);
  }
  msgs__msg__DetectionWithGPS * ros_message = _ros_message;
  {  // header
  ...
  {  // boxes
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->boxes.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // tracking_ids
    PyObject * field = PyObject_GetAttrString(_pymsg, "tracking_ids");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      int32_t * dest = ros_message->tracking_ids.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'tracking_ids'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of DetectionWithGPS */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("msgs.msg._detection_with_gps");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "DetectionWithGPS");
  ...
  {  // boxes
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "boxes");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->boxes.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->boxes.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->boxes.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // tracking_ids
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "tracking_ids");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->tracking_ids.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int32_t * src = &(ros_message->tracking_ids.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->tracking_ids.size * sizeof(int32_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
